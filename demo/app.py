"""
Gradio Interactive Web Application Dashboard.

Integrates Model Registry, Feature Store, A/B Testing engines, and 
Streaming queues under a highly interactive, custom themed web interface.
Exposes personalized scoring tables, experiment simulations, and Flink queues.
"""
...
# Align system path to import modules cleanly
...
def _get_pipeline() -> TrainingPipeline:
    """Retrieves or lazy-initializes a lightweight TrainingPipeline instance."""
    global _pipeline
    if _pipeline is None:
        print("🌱 Quick-training a small pipeline instance for the Gradio UI...")
        _pipeline = TrainingPipeline(num_users=300, num_items=1000, num_interactions=10000)
...
""")
        ...
        with gr.Tab("🎯 Get Recommendations"):
            ...
            with gr.Row():
                with gr.Column():
                    user_id_input = gr.Slider(0, 499, value=42, step=1, label="User ID")
...
def launch_demo(pipeline: Optional[TrainingPipeline] = None):
    """Launches the interactive Gradio demo application at localhost:7860."""
    global _pipeline
